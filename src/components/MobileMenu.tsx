...
export const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, onClose }) => {
  ...
  const serviceSubLinks = [
    { name: "Signage Manufacturing", href: "/signage", icon: Layers, desc: "3D SS, Acrylic, Brass, Safety Signs" },
    { name: "Interior Works", href: "/interior-works", icon: Compass, desc: "Turnkey Office & Showroom Fit-outs" },
    { name: "Real Estate", href: "/real-estate", icon: Building2, desc: "Commercial Properties & Plots" },
  ];
  ...
  return (
    <div className="fixed inset-0 z-[100] lg:hidden flex justify-end">
      ...
      {/* Sleek Compact Drawer Panel */}
      <div className="relative w-full max-w-[270px] sm:max-w-xs h-full bg-white border-l border-slate-200 shadow-2xl flex flex-col justify-between overflow-y-auto p-3.5 pb-20 z-10 space-y-3 animate-in slide-in-from-right duration-300">
        <div className="space-y-3">
          {/* Header row */}
          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
            <Link href="/" onClick={onClose} className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-brand-red flex items-center justify-center font-black text-white text-xs shadow-xs shrink-0">
                SE
              </div>
              <div className="flex flex-col">
                <span className="font-black text-slate-900 tracking-tight text-xs uppercase leading-none">
                  SRIVATHSA
                </span>
                <span className="text-[8.5px] text-slate-500 font-bold uppercase tracking-wider leading-none mt-0.5">
                  ENTERPRISES
                </span>
              </div>
            </Link>
          ...
          <nav className="space-y-0.5">
            ...
            <div className="rounded-md border border-slate-100 overflow-hidden bg-slate-50/50 my-0.5">
              <button
                ...
                className="w-full flex items-center justify-between px-2.5 py-1.5 text-xs font-semibold text-slate-900 bg-slate-100/60 hover:bg-slate-100"
              >
                  ...
                  {serviceSubLinks.map((sub) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div>
                          <div className="text-[11px] font-bold text-slate-900 leading-tight">{sub.name}</div>
                          <div className="text-[9px] text-slate-500 line-clamp-1 leading-snug">{sub.desc}</div>
                        </div>
                      </Link>
                    );
                  })}
          ...
          <div className="border-t border-slate-100 pt-2.5 space-y-2">
            <Link
              href="/get-a-quote"
              onClick={onClose}
              className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-md bg-brand-red hover:bg-brand-dark-red text-white font-extrabold text-center text-xs shadow-2xs transition-all uppercase tracking-wider"
            >
              Get a Quote
            </Link>
            ...
            <WhatsAppButton text="WhatsApp Quick Query" className="w-full text-[10px] py-1.5 px-2.5" />
          </div>
        </div>
      </div>
    </div>
